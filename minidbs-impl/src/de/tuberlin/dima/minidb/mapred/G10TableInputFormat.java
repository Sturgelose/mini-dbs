...
public class G10TableInputFormat extends TableInputFormat {
	...
	public RecordReader<Text, DataTuple> createRecordReader(InputSplit split,
		...
		return new RecordReader<Text, DataTuple>() {
			...
			public void initialize(InputSplit split, TaskAttemptContext context)
				...
				if (! (split instanceof G10InputSplit))
					throw new IOException("Bad input split type : " + split.getClass());
				
				table = TableInputFormat.getDBInstance().getCatalogue().getTable(((G10InputSplit)split).getTableName()).getResourceManager();
				resId = TableInputFormat.getDBInstance().getCatalogue().getTable(((G10InputSplit)split).getTableName()).getResourceId();

				key = new Text(((G10InputSplit)split).getTableName());
				
				firstPage = ((G10InputSplit)split).getFirstPageNumber();
				lastPage = ((G10InputSplit)split).getLastPageNumber();
		...
		if (splitSize != -1) {
			...
			for (String tableName : tableNames) {
				...
				for (int i = 0; i < nbrSplits; i++) {
					splits.add(new G10InputSplit(tableName, firstPage + i*splitSize, Math.min(firstPage + (i+1) * splitSize - 1, lastPage)));
				}			
			}	
